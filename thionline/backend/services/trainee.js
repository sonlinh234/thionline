...
let traineeenter = (req,res,next)=>{
    ...
    req.check('name','Trường này là bắt buộc.').notEmpty();
    req.check('contact','Liên hệ không hợp lệ.').isLength({min : 8,max :13}).isNumeric({no_symbols: false});
    ...
    if(errors){
        ...
        TestPaperModel.findOne({ _id : testid, isRegistrationavailable : true }).then((info)=>{
            if(info){
                TraineeEnterModel.findOne({$or:[{emailid : emailid , testid : testid},{contact : contact, testid : testid}]}).then((data)=>{
                    if(data){
                        ...
                        tempdata.save().then((u)=>{
                                sendmail(emailid,"Đăng ký thành công",`Bạn đã đăng ký thành công. Click vào link để kiểm tra  "${req.protocol + '://' + "localhost:3000"}/trainee/taketest?testid=${testid}&traineeid=${u._id}"`).then((dd)=>{
                                    console.log(dd)
                                }).catch((errr)=>{
                                    console.log(errr);
                                })
                ...
                res.json({
                    success : false,
                    message : ` Bài kiểm tra này đã đóng đăng ký!`
                })
            }
    ...
    .populate({
        ...
        if(err){
            ...
            if(!correctAnswers){
                res.json({
                    success : false,
                    message : 'Bài kiểm tra không hợp lệ.'
                })
...
let checkFeedback = (req,res,next)=>{
    ...
    FeedbackModel.findOne({userid:userid,testid:testid}).then((info)=>{
        if(!info){
            res.json({
                success : true,
                message : 'Phản hồi không thuộc user này',
                status : false
            })
        }else{
            res.json({
...
let resendmail = (req,res,next)=>{
    ...
    TraineeEnterModel.findById(userid,{emailid:1,testid : 1}).then((info)=>{
        if(info){
            console.log(info)
            sendmail(info.emailid,"Đăng ký thành công",`Bạn đã đăng ký thành công cho bài kiểm tra. Nhấp vào liên kết được cung cấp để làm bài kiểm tra  "${req.protocol + '://' + "localhost:3000"}/trainee/taketest?testid=${info.testid}&traineeid=${info._id}"`).then((dd)=>{
                console.log(dd)
            }).catch((errr)=>{
                console.log(errr);
            })
