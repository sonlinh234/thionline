    ...
    createtest =()=>{
        SecurePost({
            ...
            if(response.data.success){
                Alert('success','Tạo đề thành công!','Bạn sẽ được chuyển hướng đến trang Tiến hành kiểm tra.');
                setTimeout(()=>{
                    this.setState({
...
function QuestionView(props) {
    ...
    return (
        <div style={{marginBottom:'20px'}}>
            <div>
                <div style={{width:'100%'}}>
                    <b style={{float:'left'}}>Câu hỏi số. {no})</b>
                    <b style={{float:'right'}}>Điểm. {obj[0].weightage}</b>
                </div>
