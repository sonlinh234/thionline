    ...
    componentDidMount(){
        ...
        Promise.all([p1,p2,p3,p4,p5]).then((response)=>{
            ...
            if(response[0].data.success && response[1].data.success && response[2].data.success && response[3].data.success&& response[4].data.success){
                ...
                Alert('error','Lỗi !',response[0].data.message +response[1].data.message+response[2].data.message)
            }
        }).catch((error)=>{
            console.log(error);
            Alert('error','Lỗi!','Lỗi máy chủ.')
        })
    }
            ...
            return (
                <div>
                    <Tabs defaultActiveKey="1" onChange={ (e)=>this.tabChange(e)}>
                        <TabPane tab={ <span><Icon type="home" />Thông tin đề</span> } key="1">
                            <Descriptions bordered title="" border size="small" column={{ xxl: 1, xl: 1, lg: 1, md: 1, sm: 1, xs: 1 }}>
                                <Descriptions.Item label="id">{this.props.trainer.DataActiveTestDetails.testDetailsId}</Descriptions.Item>
                                <Descriptions.Item label="Link"><Input disabled={true} value={`${this.state.mainlink}user/conducttest?testid=${id}`} addonAfter={<CopyToClipboard text={`${this.state.mainlink}user/conducttest?testid=${id}`} onCopy={()=>message.success('Đã chép vào clipboard')}><Icon type="copy"/></CopyToClipboard>}/></Descriptions.Item>
                                <Descriptions.Item label="Tên">{testdetails.title}</Descriptions.Item>
                                <Descriptions.Item label="Loại">{testdetails.type === 'pre-test' ? 'Kiểm tra kiến thức' : 'Kiểm tra cuối kỳ'}</Descriptions.Item>
                                ...
                                <Descriptions.Item label="Tạo vào">{moment(testdetails.createdAt).format("DD/ MM/YYYY")}</Descriptions.Item>
                            </Descriptions>
                        </TabPane>
